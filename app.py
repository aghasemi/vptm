...
st.markdown(f''' 
Many neural network architectures have been proposed for solving computer vision tasks such as object recognition and image categorisation. Most of them are released along with pretrained weights obtained from training on ImageNet and similar datsets, making them a very good match for use as feature extractors in a transfer learning scenario for a much wider variety of tasks.
...
For these reasons, and initially for personal use, I developed this bubble chart to more easily visualise and compare the differences between various pretrained model available in the [PyTorch Image Models](https://github.com/rwightman/pytorch-image-models) library (version {timm.__version__}). One can choose various dimensions for the horizontal and vertical axes as well as colour, size, and shape of the bubbles. It is also possible to filter out certain architectures.
